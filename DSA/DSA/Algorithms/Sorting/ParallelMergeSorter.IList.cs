...
namespace DSA.Algorithms.Sorting
{
    public static partial class ParallelMergeSorter
    {
        ...
        public static IList<T> ParallelMergeSort<T>(this IList<T> list)
        {
        ...
        /// <param name="list">The <see cref="IList{T}"/> containing the elements for sorting.</param>
        /// <returns>Returns the given <see cref="IList{T}"/> when sorted.</returns>
        public static IList<T> ParallelMergeSortDescending<T>(this IList<T> list)
        {
            if (list.Count == 0) return list;
        ...
        /// <returns>Returns the given <see cref="IList{T}"/> when sorted.</returns>
        public static IList<T> ParallelMergeSort<T>(this IList<T> list, Comparison<T> comparison)
        {
        ...
        /// <param name="comparison">The <see cref="Comparison{T}"/> used for comparing the elements.</param>
        /// <returns>Returns the given <see cref="IList{T}"/> when sorted.</returns>
        public static IList<T> ParallelMergeSortDescending<T>(this IList<T> list, Comparison<T> comparison)
        {
            if (comparison == null) throw new ArgumentNullException(nameof(comparison));
        ...
        /// <returns>Returns the given <see cref="IList{T}"/> when sorted.</returns>
        public static IList<T> ParallelMergeSort<T>(this IList<T> list, IComparer<T> comparer)
        {
            if (list.Count == 0) return list;

            return ParallelMergeSort(list, 0, list.Count, comparer);
        }
        ...
        /// or null to use the default comparer <see cref="Comparer{T}.Default"/>.</param>
        /// <returns>Returns the given <see cref="IList{T}"/> when sorted.</returns>
        public static IList<T> ParallelMergeSortDescending<T>(this IList<T> list, IComparer<T> comparer)
        {
            if (list.Count == 0) return list;

            return ParallelMergeSortDescending(list, 0, list.Count, comparer);
        }
        ...
        /// or null to use the default comparer <see cref="Comparer{T}.Default"/>.</param>
        /// <returns>Returns the given <see cref="IList{T}"/> when sorted.</returns>
        public static IList<T> ParallelMergeSort<T>(this IList<T> list, int index, int count, IComparer<T> comparer)
        {
            ...
            ParallelSplitMerge(workArray, index, index + count, list, depth, comparer);

            return list;
        }

        /// <summary>
        /// Parallel algorithm. Sorting the elements from the source <see cref="IList{T}"/> and saving them in the destionation <see cref="IList{T}"/>. Used for ascending sort.
        /// </summary>
        /// <typeparam name="T">The data type of the <see cref="IList{T}"/>.</typeparam>
        ...
        private static void ParallelSplitMerge<T>(IList<T> src, int begin, int end, IList<T> dst, int depthRemaining, IComparer<T> comparer)
        {
            ...
            if (depthRemaining > 0)
            {
                Parallel.Invoke
                    (
                        () => ParallelSplitMerge(dst, begin, middle, src, depthRemaining - 1, comparer),
            ...
            {
                SequentialSplitMerge(dst, begin, middle, src, comparer);
                SequentialSplitMerge(dst, middle, end, src, comparer);
            }
        ...
        /// <summary>
        /// Sequential algorithm. Sorting the elements from the source <see cref="IList{T}"/> and saving them in the destionation <see cref="IList{T}"/>. Used for ascending sort.
        /// </summary>
        /// <typeparam name="T">The data type of the <see cref="IList{T}"/>.</typeparam>
        /// <param name="src">The source <see cref="IList{T}"/> containing the elements for sorting.</param>
        /// <param name="dst">The destination <see cref="IList{T}"/> for saving the sorted elements.</param>
        ...
        /// <param name="end">The exclusive end of the current split.</param>
        /// <param name="comparer">The <see cref="IComparable{T}"/> implementation used for comparing the elements.</param>
        private static void SequentialSplitMerge<T>(IList<T> src, int begin, int end, IList<T> dst, IComparer<T> comparer)
        {
            // End of recursion. If we have 1 item or less we consider it sorted
            if (end - begin < 2) return;
        ...
        /// or null to use the default comparer <see cref="Comparer{T}.Default"/>.</param>
        /// <returns>Returns the given <see cref="IList{T}"/> when sorted.</returns>
        public static IList<T> ParallelMergeSortDescending<T>(this IList<T> list, int index, int count, IComparer<T> comparer)
        {
            if (index < 0 || index >= list.Count) throw new ArgumentOutOfRangeException(nameof(index));
            if (count < 0) throw new ArgumentOutOfRangeException(nameof(count));
            if (index + count > list.Count) throw new ArgumentException("Invalid length specified.");
            ...
            // Recursive merge sort using the workList as a source and our list as a destination for the sorted items
            ParallelSplitMergeDescending(workList, index, index + count, list, depth, comparer);

            return list;
        }

        /// <summary>
        /// Parallel algorithm. Sorting the elements from the source <see cref="IList{T}"/> and saving them in the destionation <see cref="IList{T}"/>. Used for descending sort.
        /// </summary>
        /// <typeparam name="T">The data type of the <see cref="IList{T}"/>.</typeparam>
        /// <param name="src">The source <see cref="IList{T}"/> containing the elements for sorting.</param>
        /// <param name="dst">The destination <see cref="IList{T}"/> for saving the sorted elements.</param>
        ...
        /// <param name="depthRemaining">The remaining depth of the recursion for which parallel tasks are invoked.</param>
        /// <param name="comparer">The <see cref="IComparable{T}"/> implementation used for comparing the elements.</param>
        private static void ParallelSplitMergeDescending<T>(IList<T> src, int begin, int end, IList<T> dst, int depthRemaining, IComparer<T> comparer)
        {
            // End of recursion. If we have 1 item or less we consider it sorted
            if (end - begin < 2) return;
            ...
            if (depthRemaining > 0)
            {
                Parallel.Invoke
                    (
                        () => ParallelSplitMergeDescending(dst, begin, middle, src, depthRemaining - 1, comparer),
                        () => ParallelSplitMergeDescending(dst, middle, end, src, depthRemaining - 1, comparer)
                    );
            }
            else // if max depth level is exceeded we continue with sequential spliting
            {
                SequentialSplitMergeDescending(dst, begin, middle, src, comparer);
                SequentialSplitMergeDescending(dst, middle, end, src, comparer);
            }
        ...
        /// <summary>
        /// Sequential algorithm. Sorting the elements from the source <see cref="IList{T}"/> and saving them in the destionation <see cref="IList{T}"/>. Used for descending sort.
        /// </summary>
        /// <typeparam name="T">The data type of the <see cref="IList{T}"/>.</typeparam>
        /// <param name="src">The source <see cref="IList{T}"/> containing the elements for sorting.</param>
        /// <param name="dst">The destination <see cref="IList{T}"/> for saving the sorted elements.</param>
        ...
        /// <param name="end">The exclusive end of the current split.</param>
        /// <param name="comparer">The <see cref="IComparable{T}"/> implementation used for comparing the elements.</param>
        private static void SequentialSplitMergeDescending<T>(IList<T> src, int begin, int end, IList<T> dst, IComparer<T> comparer)
        {
            // End of recursion. If we have 1 item or less we consider it sorted
            if (end - begin < 2) return;
