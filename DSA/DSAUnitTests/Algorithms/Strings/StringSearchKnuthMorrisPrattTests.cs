...
{
    [TestClass]
    public class StringSearchKnuthMorrisPrattTests
    {
        [TestMethod]
        public void SinglePatternFirstOccurrence()
        {
            ...
            Assert.IsTrue(StringSearch.KnuthMorrisPrattSearchFirst(t, p) == 16);

            t = "test1 test2 test3 test4";
            p = "test2";

            Assert.IsTrue(StringSearch.KnuthMorrisPrattSearchFirst(t, p) == 6);

            t = "кирилица";
            p = "ли";

            Assert.IsTrue(StringSearch.KnuthMorrisPrattSearchFirst(t, p) == 4);

            t = "no such pattern";
            p = "smth";

            Assert.IsTrue(StringSearch.KnuthMorrisPrattSearchFirst(t, p) == -1);
        }
        ...
        public void SinglePatternAllOccurrences()
        {
            ...
            var positions = StringSearch.KnuthMorrisPrattSearchAll(t, p);
            ...
            positions = StringSearch.KnuthMorrisPrattSearchAll(t, p);
            Assert.IsTrue(positions.Count == expectedOccur.Length);
            for (int i = 0; i < expectedOccur.Length; i++)
                Assert.IsTrue(positions[i] == expectedOccur[i]);
        ...
        public void MultiplePatternsFirstOccurence()
        {
            ...
            var positions = StringSearch.KnuthMorrisPrattMultipleSearchFirst(t, p);
            Assert.IsTrue(positions.Count == expectedOccur.Length);
            for (int i = 0; i < expectedOccur.Length; i++)
            ...
            positions = StringSearch.KnuthMorrisPrattMultipleSearchFirst(t, p);
            Assert.IsTrue(positions.Count == expectedOccur.Length);
            for (int i = 0; i < expectedOccur.Length; i++)
                Assert.IsTrue(positions[p[i]] == expectedOccur[i]);
        ...
        public void MultiplePatternsAllOccurrences()
        {
            ...
            var positions = StringSearch.KnuthMorrisPrattMultipleSearchAll(t, p);
            ...
            positions = StringSearch.KnuthMorrisPrattMultipleSearchAll(t, p);
            for (int i = 0; i < p.Length; i++)
            {
                Assert.IsTrue(positions[p[i]].Count == expectedOccur[i].Length);
                for (int j = 0; j < expectedOccur[i].Length; j++)
