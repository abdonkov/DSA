...
{
    [TestClass]
    public class StringSearchRabinKarpTests
    {
        [TestMethod]
        public void SinglePatternFirstOccurrence()
        {
            ...
            Assert.IsTrue(StringSearch.RabinKarpSearchFirst(t, p) == 16);

            t = "test1 test2 test3 test4";
            p = "test2";

            Assert.IsTrue(StringSearch.RabinKarpSearchFirst(t, p) == 6);

            t = "кирилица";
            p = "ли";

            Assert.IsTrue(StringSearch.RabinKarpSearchFirst(t, p) == 4);

            t = "no such pattern";
            p = "smth";

            Assert.IsTrue(StringSearch.RabinKarpSearchFirst(t, p) == -1);
        }
        ...
        public void MultiplePatternsAllOccurrences()
        {
            ...
            var positions = StringSearch.RabinKarpMultipleSearchAll(t, p);
            ...
            positions = StringSearch.RabinKarpMultipleSearchAll(t, p);
            for (int i = 0; i < p.Length; i++)
            {
                Assert.IsTrue(positions[p[i]].Count == expectedOccur[i].Length);
                for (int j = 0; j < expectedOccur[i].Length; j++)
